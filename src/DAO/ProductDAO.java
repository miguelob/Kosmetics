...
//    }
    ...
    public static ArrayList<Product> getBestSales() {
        ...
        try {
            con = ConnectionDAO.getInstance().getConnection();
            PreparedStatement pst = con.prepareStatement("SELECT Products_idProducts FROM invoice GROUP BY Products_idProducts ORDER BY COUNT(*) DESC LIMIT 10;");
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                products.add(ProductDAO.getProductFromId(rs.getInt(1)));
            }
